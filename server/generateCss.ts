...
function getFontWeightNum(weight: string) {
...
function getCss(id, fontName, fontWeight, url, unicode): string {
...
export async function generateFontCss(family: string, weight: number[]): Promise<string> {
  return new Promise((resolve, reject) => {
    if (!Object.keys(allFamily).includes(family))
      return reject(new Error('not exist'))
    weight.sort((a, b) => a - b)
    const namedWeight: string[] = []
    weight.forEach((item) => {
      const w = config[item]
      for (const i of w) {
