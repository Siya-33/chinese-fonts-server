...
function getFontWeightNum(weight) {
  for (const [num, nameWeight] of Object.entries(config)) {
    if (nameWeight.includes(weight))
      return num;
  }
}
const allFamily = {
...
function getCss(id, fontName, fontWeight, url, unicode) {
  return `/* ${id} ${fontWeight}*/
  @font-face {
...
async function generateFontCss(family, weight) {
  return new Promise((resolve, reject) => {
    if (!Object.keys(allFamily).includes(family))
    ...
    weight.forEach((item) => {
      const w = config[item];
      for (const i of w) {
        if (allFamily[family].includes(i)) {
          namedWeight.push(i);
          break;
        }
      }
    });
    const output = `/css/${family}_${namedWeight.join("_")}.css`;
    const savePath = path__default.join(process__default.cwd(), "lib", output);
    if (fs__default.existsSync(savePath))
      return resolve(output);
    const fontFace = [];
    const entries = Object.entries(ranges);
    for (const fontWeight of namedWeight) {
    ...
    fs__default.writeFileSync(`${savePath}`, ccsText, "utf-8");
    resolve(output);
  });
}
...
app.get("/", async (req, res) => {
  ...
  try {
    const [family, weight] = req.query.family.split(":wght@");
    ...
    const fontPath = path__default.join(process__default.cwd(), "lib", url);
    res.set({
      "content-type": "text/css;charset=UTF-8",
      "etag": false,
      "last-modified": false,
      "Cache-Control": `max-age=${60 * 60 * 1e3}`
    });
